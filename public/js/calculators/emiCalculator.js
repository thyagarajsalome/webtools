...
export const init = function (db, showToast, shareResults, showInstallPrompt) {
  ...
  function displayResults(data) {
    ...
    resultsSection.innerHTML = `
            ...
            <div class="results-card" id="results-card">
                <div class="results-grid">
                    ...
                    <div>
                        <p>Monthly EMI</p>
                        <p id="grandTotal">₹${Math.round(data.emi).toLocaleString("en-IN")}</p>
                        <div class="summary-details">
                            <div class="summary-item"><span>Principal Amount:</span><span>₹${Math.round(data.principal).toLocaleString("en-IN")}</span></div>
                            <div class="summary-item"><span>Total Interest Paid:</span><span>₹${Math.round(data.totalInterest).toLocaleString("en-IN")}</span></div>
                            <div class="summary-item summary-item-total"><span>Total Payment:</span><span>₹${Math.round(data.totalPayment).toLocaleString("en-IN")}</span></div>
                        </div>
                    </div>
                </div>
    ...
    document.getElementById("saveBtn").addEventListener("click", () => {
      const name = prompt("Enter a name for this calculation:", "My Home Loan EMI");
      if (name) {
        const project = {
          id: `proj_${Date.now()}`,
          name: name,
