...
export const init = function (db, showToast, shareResults, showInstallPrompt) {
  ...
  const roomDefaults = {
    Bedroom: { lights: 3, fans: 1, sockets6A: 4, sockets16A: 1, acs: 1, tvs: 1 },
    "Living Room": { lights: 4, fans: 2, sockets6A: 5, sockets16A: 1, acs: 1, tvs: 1 },
    Kitchen: { lights: 2, fans: 0, sockets6A: 3, sockets16A: 3, acs: 0, tvs: 0 },
    Bathroom: { lights: 2, fans: 0, sockets6A: 1, sockets16A: 1, acs: 0, tvs: 0 },
    Balcony: { lights: 1, fans: 0, sockets6A: 1, sockets16A: 0, acs: 0, tvs: 0 },
    Staircase: { lights: 2, fans: 0, sockets6A: 1, sockets16A: 0, acs: 0, tvs: 0 },
  };

  function updateStepVisibility() {
    document.querySelectorAll(".step").forEach((step, index) => step.classList.toggle("active", index + 1 === currentStep));
    document.querySelectorAll(".step-btn").forEach((btn, index) => btn.classList.toggle("active", index < currentStep));
  ...
  function createInput(name, label, id, value) {
    return `<div class="input-group"><label>${label}</label><input type="number" data-room-id="${id}" data-point-type="${name}" value="${value}" min="0" style="width: 100%; text-align: center; padding: 0.5rem;"></div>`;
  }

  function renderRooms() {
    const roomContainer = document.getElementById("room-container");
    roomContainer.innerHTML = rooms.map(
        (room) => `
        <div id="${room.id}" class="item-card">
            <div class="item-card-header">
                <h3>${room.type}</h3>
                <button type="button" class="remove-item-btn" data-id="${room.id}">Remove</button>
            </div>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); gap: 1rem; font-size: 0.875rem;">
                ${createInput("lights", "Lights", room.id, room.lights)}
                ${createInput("fans", "Fans", room.id, room.fans)}
                ${createInput("sockets6A", "6A Sockets", room.id, room.sockets6A)}
                ${createInput("sockets16A", "16A Sockets", room.id, room.sockets16A)}
                ${createInput("acs", "AC Points", room.id, room.acs)}
  ...
  function calculateBudget(e) {
    ...
    const quality = document.querySelector('input[name="brandPreference"]:checked').value;
    const laborPerPoint = parseFloat(document.getElementById("laborPerPoint").value) || 0;
    const mainPanelLabor = parseFloat(document.getElementById("mainPanelLabor").value) || 0;
    
    const totalPoints = rooms.reduce(
      (acc, room) => acc + room.lights + room.fans + room.sockets6A + room.sockets16A + room.acs + room.tvs,
      0
    );
    ...
    const totalPointMaterialCost = Object.values(pointCosts).reduce((a, b) => a + b, 0);
    const commonMaterialCost = (costData.commonMaterials.pipe[quality] + costData.commonMaterials.mainWire[quality] + costData.commonMaterials.db[quality]) * houseArea;
    ...
    displayResults({
      ...
      commonMaterialCost,
      houseArea,
      inputs: { houseArea, quality, laborPerPoint, mainPanelLabor, rooms: [...rooms] },
    });
  }
  ...
  function displayResults(data) {
    ...
    resultsSection.innerHTML = `
        ...
        <div class="results-card" id="results-card">
            <div class="results-grid">
                ...
                <div>
                    <p>Total Estimated Cost</p><p id="grandTotal">₹${Math.round(data.grandTotal).toLocaleString("en-IN")}</p>
                    <div class="summary-details">
                        <div class="summary-item"><span>Total Material Cost:</span><span id="totalMaterialCost">₹${Math.round(data.totalMaterialCost).toLocaleString("en-IN")}</span></div>
                        <div class="summary-item"><span>Total Labor Cost:</span><span id="totalLaborCost">₹${Math.round(data.totalLaborCost).toLocaleString("en-IN")}</span></div>
                        <div class="summary-item summary-item-total"><span>Total Electrical Points:</span><span id="totalPoints">${data.totalPoints}</span></div>
                    </div>
                </div>
            </div>
            <div class="detailed-breakdown-container">
                ...
                <div id="detailed-breakdown">
                    <div class="summary-item"><span>Light Points Cost:</span> <span>₹${Math.round(data.pointCosts.lights).toLocaleString("en-IN")}</span></div>
                    <div class="summary-item"><span>Fan Points Cost:</span> <span>₹${Math.round(data.pointCosts.fans).toLocaleString("en-IN")}</span></div>
                    <div class="summary-item"><span>6A Socket Points Cost:</span> <span>₹${Math.round(data.pointCosts.sockets6A).toLocaleString("en-IN")}</span></div>
                    <div class="summary-item"><span>16A Socket Points Cost:</span> <span>₹${Math.round(data.pointCosts.sockets16A).toLocaleString("en-IN")}</span></div>
                    <div class="summary-item"><span>AC & TV Points Cost:</span> <span>₹${Math.round(data.pointCosts.acs + data.pointCosts.tvs).toLocaleString("en-IN")}</span></div>
                    <div class="summary-item summary-item-total"><span>Pipes, Wires, DB etc. (based on ${data.houseArea} sq.ft.):</span> <span>₹${Math.round(data.commonMaterialCost).toLocaleString("en-IN")}</span></div>
                </div>
            </div>
        </div>`;
    ...
    document.getElementById("saveBtn").addEventListener("click", () => {
      const name = prompt("Enter a name for this project:", "My Electrical Plan");
      if (name) {
        const defaultTasks = [
          { id: `task_${Date.now() + 1}`, text: "Hire an electrician", done: false },
          { id: `task_${Date.now() + 2}`, text: "Finalize point locations", done: false },
          { id: `task_${Date.now() + 3}`, text: "Buy wires and conduit pipes", done: false },
          { id: `task_${Date.now() + 4}`, text: "Buy switches and fixtures", done: false },
          { id: `task_${Date.now() + 5}`, text: "Install main DB and earthing", done: false },
        ];
  ...
  function renderChart(material, labor) {
    ...
    chartInstance = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{ data: [material, labor], backgroundColor: ["#4f46e5", "#10b981"], borderColor: "#ffffff", borderWidth: 4 }],
      },
      options: { responsive: true, cutout: "70%", plugins: { legend: { position: "bottom", labels: { padding: 20, font: { size: 14 } } } } },
    });
  }
  ...
  document.getElementById("electricalForm").addEventListener("submit", calculateBudget);
